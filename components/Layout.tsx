...
import { User, CurrencyCode, Notification, SystemMode, AppTheme } from '../types';
...
const Layout: React.FC<LayoutProps> = ({ 
  ...
  return (
    <div className={`flex h-screen overflow-hidden font-sans transition-all duration-700 ${isTrainingMode ? 'border-4 border-amber-500/30' : ''}`}>
      
      {systemMode === 'MODERN' && (
        <aside className={`bg-white dark:bg-[var(--app-bg)] border-r border-slate-200 dark:border-white/[0.05] transition-all duration-500 flex flex-col z-40 ${isSidebarOpen ? 'w-64' : 'w-24'}`}>
          <div className="p-6">
            {isSidebarOpen ? (
          ...
          <div className="p-6 border-t border-slate-200 dark:border-white/[0.05]">
            <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className="w-full flex items-center justify-center p-3 rounded-xl bg-slate-100 dark:bg-black/20 hover:bg-slate-200 dark:hover:bg-white/10 text-slate-500 transition-all">
      ...
      <main className="flex-1 flex flex-col overflow-hidden relative">
        <header className={`h-20 border-b border-slate-200 dark:border-[var(--glass-border)] bg-white/50 dark:bg-[var(--glass-bg)] backdrop-blur-2xl flex items-center justify-between px-10 z-30 transition-colors ${isTrainingMode ? 'bg-amber-500/5' : ''}`}>
          <div className="flex items-center gap-6">
            ...
            <div className="flex items-center gap-2 px-3 py-1.5 bg-slate-100 dark:bg-[var(--app-bg)] rounded-2xl border border-slate-200 dark:border-[var(--glass-border)]">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="relative group">
              ...
              <select 
                ...
                className="pl-9 pr-8 py-2 bg-white dark:bg-[var(--app-bg)] border border-slate-200 dark:border-[var(--glass-border)] rounded-xl text-[10px] font-black uppercase tracking-widest outline-none focus:ring-2 focus:ring-blue-500/20 transition-all text-slate-700 dark:text-slate-200 appearance-none shadow-sm cursor-pointer min-w-[120px]"
              >
            ...
            <div className="relative">
              <button 
                ...
                className="p-3 rounded-xl bg-white dark:bg-[var(--app-bg)] border border-slate-200 dark:border-[var(--glass-border)] text-slate-500 hover:text-blue-500 transition-all relative"
              >
                <Bell size={18} />
              ...
              {isNotifyOpen && (
                <div className="absolute right-0 mt-4 w-96 bg-white dark:bg-[var(--app-bg)] border border-slate-200 dark:border-[var(--glass-border)] rounded-3xl shadow-2xl z-50 glass-card animate-in fade-in slide-in-from-top-4">
                  <div className="p-6 border-b border-slate-100 dark:border-[var(--glass-border)] flex justify-between items-center">
            ...
            <div className="relative group">
              ...
              <select 
                value={theme}
                onChange={(e) => onToggleTheme(e.target.value as AppTheme)}
                className="pl-9 pr-8 py-2 bg-white dark:bg-[var(--app-bg)] border border-slate-200 dark:border-[var(--glass-border)] rounded-xl text-[10px] font-black uppercase tracking-widest outline-none focus:ring-2 focus:ring-blue-500/20 transition-all text-slate-700 dark:text-slate-200 appearance-none shadow-sm cursor-pointer min-w-[120px]"
              >
                <option value="light">Light UI</option>
                <option value="midnight">Midnight UI</option>
                <option value="obsidian">Obsidian UI</option>
                <option value="forest">Forest UI</option>
                <option value="amber">Amber Gold</option>
                <option value="amethyst">Royal Amethyst</option>
                <option value="crimson">Cyber Crimson</option>
              </select>
              <div className="absolute inset-y-0 right-3 flex items-center pointer-events-none text-slate-400">
                <ChevronDown size={12} />
              </div>
            ...
            <div className="relative">
              ...
              {isProfileOpen && (
                <div className="absolute right-0 mt-4 w-64 bg-white dark:bg-[var(--app-bg)] border border-slate-200 dark:border-[var(--glass-border)] rounded-3xl shadow-2xl p-3 z-50 glass-card animate-in fade-in slide-in-from-top-4">
                  <div className="p-4 border-b border-slate-100 dark:border-[var(--glass-border)] mb-2">
